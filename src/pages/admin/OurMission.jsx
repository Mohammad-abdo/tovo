...
import { FiSearch, FiPlus, FiTarget } from 'react-icons/fi'
...
const OurMission = () => {
  ...
  const fetchItems = async () => {
    try {
      const response = await api.get('/frontend-data?type=our_mission')
      if (response.data.success) {
        setItems(response.data.data || [])
      }
    } catch (error) {
      console.error('Error fetching our mission items:', error)
    } finally {
      setLoading(false)
    }
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h1 className="text-3xl font-bold text-gray-900 dark:text-white">{t('ourMission', language) || 'Our Mission'}</h1>
          <p className="text-gray-600 dark:text-gray-400 mt-1">Manage our mission content</p>
        </div>
        <button
      ...
      <Modal
        ...
        title={editingItem ? 'Edit Our Mission Item' : 'Add Our Mission Item'}
        size="lg"
      >
